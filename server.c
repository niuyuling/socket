...
void _accept(int server_socket, conf * p)
{
    ...
    while (1) {
        ...
        if (strcmp(clnt_ip, LO) == 0 || strcmp(clnt_ip, p->clientip) == 0
            || strcmp(clnt_ip, p->clientip1) == 0) {
...
{
    char *inifile = "conf/config.ini";
    conf *p = (struct CONF *)malloc(sizeof(struct CONF));
    read_conf(inifile, p);
    ...
    serv_addr.sin_addr.s_addr = inet_addr(p->ip); // 具体的IP地址
    serv_addr.sin_port = htons(p->port); // 端口
